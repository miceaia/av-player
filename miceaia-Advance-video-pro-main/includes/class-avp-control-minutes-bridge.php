 ...
 */
class AVP_Control_Minutes_Bridge {
    public const CLIENT_SLUG = 'advanced-video-player';

    /**
     * @var AVP_Control_Minutes_Bridge|null
     */
    private static ?AVP_Control_Minutes_Bridge $instance = null;

    /**
     * Obtiene la instancia singleton.
     *
     * @return AVP_Control_Minutes_Bridge
     */
     ...
     * @return array
     */
    public function inject_localized_data(array $data): array {
        $config = array(
            'enabled' => false,
    ...
    public function handle_event(): void {
        ...
        if (isset($_POST['events'])) {
            ...
            $events[] = $this->sanitize_event((array) $_POST);
        }
     ...
     * @return array
     */
    private function normalize_events($events): array {
        if (is_string($events)) {
     ...
     * @return array
     */
    private function sanitize_event(array $source): array {
        $event = array(
        ...
        if (isset($source['meta']) && is_array($source['meta'])) {
            $event['meta'] = $this->sanitize_meta($source['meta']);
        }
